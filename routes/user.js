...
router.put("/:id",
    ...
    async (req, res) => { 
      try {
        const query = util.promisify(conn.query).bind(conn);
        const errors = validationResult(req);
        const checkEmail = await query("select * from users where email = ? and id != ?", [req.body.email,req.params.id]);
        if (!errors.isEmpty()) {
          return res.status(400).json({ errors: errors.array() });
        }
        const checkWarehouseId = await query(
          "select * from users where warehouse_id = ? and id != ?",
          [req.body.warehouse_id,req.params.id]
        );
        const checkWarehouseExist = await query(
          "select * from warehouse where id = ?",
          [req.body.warehouse_id]
        ...
        if (checkEmail.length > 0 || checkWarehouseExist >0  ) {
          return res.status(400).json({
            errors: [
              {
                msg: "email or warehouse are not valid",
              },
            ],   
          });
        } else {
          if ( checkWarehouseId.length > 0 ) {
            return res.status(400).json({
              errors: [
                {
                  msg: "Warehouse is assigned for another user  ",
                },
              ],
            });
          } else {
